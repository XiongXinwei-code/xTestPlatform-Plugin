...
public sealed class OpcUaConnectPlugin : StepPluginBase<OpcUaConnectSetting>
{
    ...
    public override string Description =>
        "建立 OPC UA 连接，支持匿名和用户名密码认证，支持多种安全策略。" +
        "Setting 字段：ConnectionName(string,表达式,连接标识名), EndpointUrl(string,表达式,服务器端点), " +
        "SecurityPolicy(枚举,None/Basic256Sha256/Aes128Sha256RsaOaep/Aes256Sha256RsaPss), " +
        "AuthMode(枚举,Anonymous/UserPassword), UserName(string,表达式), Password(string,表达式), " +
        "TimeoutMs(int,连接超时), AutoAcceptCertificate(bool,是否自动接受证书)。";
