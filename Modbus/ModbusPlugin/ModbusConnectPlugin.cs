...
public sealed class ModbusConnectPlugin : StepPluginBase<ModbusConnectSetting>
{
	...
	public override string Description => """
		...
		| 参数 | 类型 | 必填 | 默认值 | 说明 |
		|------|------|------|--------|------|
		| ConnectionName | string([ExpressionField] -> string) | 是 | Modbus1 | 连接标识名，序列内唯一 |
		| TransportType | 枚举 | 是 | TCP | 可选值：TCP, RTU |
		| IpAddress | string([ExpressionField] -> string) | TCP 时 | 127.0.0.1 | TCP 服务器地址 |
		| TcpPort | int | 否 | 502 | TCP 端口 |
		| PortName | string([ExpressionField] -> string) | RTU 时 | — | 串口名，如 COM1 |
