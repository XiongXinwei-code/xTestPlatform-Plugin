...
#### 6.1.2 资源表相关步骤的 Description 规范（强制）

凡是会向运行期资源表（或插件内部的全局连接池/会话池）注册、释放对象的步骤，其 `Description` 的 `## 行为` 小节**必须**回答下列问题，不允许让使用者靠推断：

| 侧 | 必答问题 |
|----|----------|
| 注册侧（Open / Create / Connect / Start 等） | ① 是否往表里放东西？② 用哪个字段作标识名？③ 同一个标识名再注册一次会发生什么？ |
| 释放侧（Close / Disconnect / Stop 等） | ④ 标识名不存在时是报错还是静默通过？ |
...
**第三项必须逐插件核对自己的 Executor 后再写**，判定依据：
...
- 标识名不存在时<按实际行为：不报错仅记录日志并返回 Passed / 步骤报错>
```

判定依据：看 Executor 里 `TryGet` 失败时走哪条路——只打日志照常返回 `Passed`，还是返回 `Error`。若行为由开关字段控制（如 `ReplaceIfExists`、`IgnoreIfNotFound`），必须把两种取值下的结果都写出来。

**额外约束：**

- 若注册的不是引擎资源表，而是**插件内部的静态连接池/会话池**（如 `TcpConnectionManager`、`DoipConnectionManager`），必须在说明中显式点明，并提示该池不随序列结束自动释放。
- 一次注册多个附属键（如终止符、传输层对象、超时配置）时，需在同一条中一并说明。
- **不要**在插件说明中重复描述资源表的作用域与生命周期（父子序列是否共享、何时自动释放）——这些是平台不变量，已由 AI 助手的 SystemPrompt 统一说明，写进插件等于同一句话复制 N 份。
- 严禁把某个协议插件的行为结论套用到另一个协议插件上（例如 VISA 与 LIN 的重复注册行为并不相同）。
