...
public sealed class CanReadPlugin : StepPluginBase<CanReadSetting>
{
    ...
    public override string Description =>
        ...
        "Setting 字段：ConnectionName(string,连接标识名), FilterId(string,过滤CAN ID 如 0x7E8 为空则接收任意), " +
        "ReadTimeoutMs(int,读取超时毫秒), ResultVariable(string,数据存储变量路径), IdVariable(string,ID存储变量路径)。";
