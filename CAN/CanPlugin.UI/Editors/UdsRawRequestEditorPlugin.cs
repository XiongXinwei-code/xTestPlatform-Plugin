...
public sealed class UdsRawRequestEditorPlugin : IStepEditorPlugin
{
		...
		StepEditorValidationContext context, CancellationToken cancellationToken = default)
	{
		...
		var s = (UdsRawRequestSetting)new UdsRawRequestPlugin().CreateSerializer().Deserialize(context.Setting, 1);
		...
		if (!string.IsNullOrWhiteSpace(s.ResultVariable))
		{
			if (!context.ExecutionContext.HasVariable(s.ResultVariable))
				errors.Add(StepSettingError.Warning("UDS_R002", $"变量 {s.ResultVariable} 不存在，请先创建该变量"));
			else
			{
