...
public sealed class LxiCloseEditorPlugin : IStepEditorPlugin
{
	public string StepTypeId => "IO.LxiClose";
	public string IconPath => "pack://application:,,,/LXI.StepPlugin.UI;component/Resources/Icons/lxi.png";

	public FrameworkElement CreateEditor(Step step, SequenceFile? sequenceFile)
	{
		var view = new LxiCloseEditorView();
		view.ViewModel.AttachSerializer(new LxiClosePlugin().CreateSerializer());
		...
		byte[] setting, IExpressionEvaluator evaluator, IExecutionContext context, CancellationToken ct = default)
	{
		...
		var s = setting is { Length: > 0 }
			? (LxiCloseSetting)new LxiClosePlugin().CreateSerializer().Deserialize(setting, 1)
			: new LxiCloseSetting();

		if (string.IsNullOrWhiteSpace(s.IpAddress))
			errors.Add(StepSettingError.Error("LXI_010", "IP 地址不能为空"));
