...
public sealed class LxiQueryExecutor : IStepExecutor
{
	...
	public async Task<ExecutionResult> ExecuteAsync(IExecutionContext context, CancellationToken cancellationToken = default)
	{
		try
		{
			...
			var serializer = new LxiQueryPlugin().CreateSerializer();
			var s = (LxiQuerySetting)serializer.Deserialize(step.StepSetting.Setting, step.StepSetting.SettingVersion);
			...
			context.LogAction?.Invoke($"LXI {ip} 查询: {command} -> {response}");
