...
{
    private readonly IStepSettingSerializer _serializer; private readonly bool _receive; private Step? _step; private UdpSendSetting? _setting; private bool _loading;
    public UdpEditorViewModel(IStepSettingSerializer serializer, bool receive) { _serializer = serializer; _receive = receive; }
    ...
    private void Save() { if (!_loading && _step is not null) _step.StepSetting.Setting = _serializer.Serialize(Setting); } public event PropertyChangedEventHandler? PropertyChanged; private void OnPropertyChanged([CallerMemberName] string? n=null) => PropertyChanged?.Invoke(this,new PropertyChangedEventArgs(n));
}
